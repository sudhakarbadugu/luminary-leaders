...
export default function CricketGrid() {
  ...
  const filtered = useMemo(() => {
    ...
    return filterByReadStatus(result, 'cricketer', readFilter);
  }, [eraFilter, roleFilter, searchQuery, readFilter]);
  ...
  const reset = () => { setEraFilter('All'); setRoleFilter('All'); setSearchQuery(''); setReadFilter('all'); setVisibleCount(8); };
  const hasActive = eraFilter !== 'All' || roleFilter !== 'All' || searchQuery || readFilter !== 'all';
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(280px, 100%), 1fr))', gap: 48, marginBottom: 64 }}>
            {displayed.map((cricketer, idx) => (
              <div key={cricketer.id} ref={el => { if (el) cardsRef.current[idx] = el; }} onClick={() => navigate(`/cricketer/${cricketer.id}`)} style={{ opacity: 0, cursor: 'pointer' }}>
                <div style={{ aspectRatio: '3/4', borderRadius: 8, overflow: 'hidden', position: 'relative', background: cricketer.image ? undefined : getGradient(SCIENTIST_GRADIENT_COLORS, cricketer.id) }}>
                  {cricketer.image ? (
                  ...
                  <ReadBadge id={cricketer.id} category="cricketer" />
        ...
        {visibleCount < filtered.length && (
          <div style={{ textAlign: 'center' }}>
            <button onClick={() => setVisibleCount(prev => Math.min(prev + 12, filtered.length))} style={{ border: '1px solid #282b2f', borderRadius: 99, padding: '14px 36px', background: 'transparent', color: '#282b2f', fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 500, cursor: 'pointer', transition: 'all 0.3s' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#282b2f'; e.currentTarget.style.color = '#f1f1ee'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#282b2f'; }}>Browse More Cricketers ({filtered.length - visibleCount} remaining)</button>
          </div>
        )}
      </div>
    </section>
