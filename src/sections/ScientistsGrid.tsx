          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(280px, 100%), 1fr))', gap: 48, marginBottom: 64 }}>
            {displayed.map((scientist, idx) => (
              <div key={scientist.id} ref={el => { if (el) cardsRef.current[idx] = el; }} onClick={() => navigate(`/scientist/${scientist.id}`)} style={{ opacity: 0, cursor: 'pointer' }}>
                <div style={{ aspectRatio: '3/4', borderRadius: 8, overflow: 'hidden', position: 'relative', background: scientist.image ? undefined : getGradient(SCIENTIST_GRADIENT_COLORS, scientist.id) }}>
                  {scientist.image ? (
        ...
        {visibleCount < filtered.length && (
          <div style={{ textAlign: 'center' }}>
            <button onClick={() => setVisibleCount(prev => Math.min(prev + 12, filtered.length))} style={{ border: '1px solid #282b2f', borderRadius: 99, padding: '14px 36px', background: 'transparent', color: '#282b2f', fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 500, cursor: 'pointer', transition: 'all 0.3s' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#282b2f'; e.currentTarget.style.color = '#f1f1ee'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#282b2f'; }}>Browse More Scientists ({filtered.length - visibleCount} remaining)</button>
          </div>
        )}
      </div>
    </section>
